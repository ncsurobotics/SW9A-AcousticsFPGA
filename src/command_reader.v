...
module command_reader (
  ...
  axis_to_mem uart_to_mem_inst (
    .clk(clk),
    .rst_n(rst_n),
    .big_bits(bartlett_data_in),
    .bin_valid(bartlett_valid),
    .address(bartlett_addr),
    .data_out(bartlett_write_data),
    .wr_en(bartlett_wr_en)
  );
  ...
  always @(posedge clk or negedge rst_n) begin  
    if (!rst_n) begin
      for (j = 0; j < 19; j = j + 1)
        ...
        bartlett_flipflops[bartlett_addr] <= bartlett_write_data;
      end
  end
  ...
  assign vga           = uart_flipflops[23][2:0];
