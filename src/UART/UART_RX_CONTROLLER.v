...
reg[9:0] softlock_prevention; // state machine can only be in each state for at most 1024 cycles

always @ (posedge clk or negedge reset_b) begin
    if (!reset_b) begin
        current_state <= IDLE;
		softlock_prevention <= 9'd0;
    end
    else begin
		current_state <= &softlock_prevention ? IDLE : next_state;
		softlock_prevention <= current_state != next_state ? 0 : softlock_prevention + 1;
	end
end
