...
real_part = 256 * randi([1,2], rows, cols) %randi([-32768, 32767], rows, cols); % 16-bit signed integers
imag_part = 256 * randi([1,2], rows, cols) %randi([-32768, 32767], rows, cols);
...
for i = 1:4
    for j = 1:4
        real_hex = dec2hex(typecast(int16(real(result(i, j))), 'int16'), 4); % Convert real to hex
        imag_hex = dec2hex(typecast(int16(imag(result(i, j))), 'int16'), 4); % Convert imag to hex
