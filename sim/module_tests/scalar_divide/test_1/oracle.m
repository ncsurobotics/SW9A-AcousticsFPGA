...
real_part = randi([0,4096], rows, cols) %randi([-32768, 32767], rows, cols); % 16-bit signed integers
imag_part = randi([0,4096], rows, cols) %randi([-32768, 32767], rows, cols);
...
for i = 1:rows
    for j = 1:cols
        real_hex = dec2hex(typecast(int32(real(result(i, j))), 'int32'), 8); % Convert real part to hex
        imag_hex = dec2hex(typecast(int32(imag(result(i, j))), 'int32'), 8); % Convert imag part to hex
        fprintf(fid, '%s%s\n', imag_hex, real_hex); % Write to file
    end
end
fclose(fid); % Close file
